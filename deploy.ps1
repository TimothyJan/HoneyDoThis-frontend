# HoneyDoThis-frontend/deploy.ps1
...
Write-Host "Deploying HoneyDoThis-frontend to GitHub Pages" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan

# Configuration
$REPO_NAME = "HoneyDoThis-frontend"
$BASE_HREF = "/$REPO_NAME/"
$OUTPUT_DIR = "dist/honeyDoThis/browser"
$GH_PAGES_DIR = "docs"
...
# Step 2: Install angular-cli-ghpages if not already installed
Write-Host "`n[2/5] Ensuring angular-cli-ghpages is available..." -ForegroundColor Yellow
...
$ghPagesCheck = npx --no-install angular-cli-ghpages --version 2>$null
if (-not $ghPagesCheck) {
    Write-Host "  Installing angular-cli-ghpages temporarily..." -ForegroundColor Gray
    # It will be installed temporarily by npx
}

# Step 3: Build the Angular app
Write-Host "`n[3/5] Building Angular app with base-href: $BASE_HREF" -ForegroundColor Yellow
if ($DryRun) {
...
# Step 4: Handle Angular 17+ application builder output structure
Write-Host "`n[4/5] Checking build output structure..." -ForegroundColor Yellow
...
# Step 5: Create a .nojekyll file to disable Jekyll processing
Write-Host "`n[5/5] Creating .nojekyll file..." -ForegroundColor Yellow
if ($DryRun) {
...
if ($DryRun) {
    Write-Host "  [DRY RUN] Would run: npx angular-cli-ghpages --dir=$GH_PAGES_DIR --no-silent" -ForegroundColor Gray
    Write-Host "`n[DRY RUN COMPLETE]" -ForegroundColor Cyan
} else {
    # Deploy using angular-cli-ghpages
    npx angular-cli-ghpages --dir=$GH_PAGES_DIR --no-silent
    ...
    if ($LASTEXITCODE -eq 0) {
        ...
        Write-Host "https://timothyjan.github.io/$REPO_NAME/" -ForegroundColor Cyan
